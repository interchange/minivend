...
=item accessories.asc

A simple auxiliary database keyed on the product code. It's value
is available via the C<[item-accessories]> or C<[accessories code]> tags.
This is a fixed-format database, and must be created as specified.
See I<Accessories>. The big advantage of this database is speed --
it is always retained in memory. If you have a large number (many
thousands) of items, use an I<Arbitrary Database> instead.

=item pricing.asc

The database of quantity pricing information. It must be defined as
the regular MiniVend database B<pricing>, with the product code as
the first field, and a field named B<price> (all lower case) that holds
space-separated price information in the order defined by I<PriceBreaks>.
Also subject to I<MixMatch>. In addition, this database F<may> hold
information about price adjustments -- see I<PriceAdjustment>.  This is
a database that can be in any form, including SQL/DBI if desired.  The
only requirement is the presence of the B<price> field in the proper
...
due to inability to create objects. For SQL there is nothing that can
be done -- you must have C<global> access to use data from SQL tables.
...
Any MiniVend tag (except ones using SQL) can be accessed
via the $Tag object. If you need to use SQL queries inside a Perl
element, you will have to have C<AllowGlobal> permissions and set
the C<global=1> parameter.
...
security restrictions that are controlled by I<SafeUntrap>. 
...
your catalog. And full "use strict" checking is in force.
...
carts, set the C<mv_order_receipt> variable in the form.
...
same server. If no I<Catalog> directives are present in the main
F<minivend.cfg> file, the server will read only a single catalog's
information from the one F<minivend.cfg> file, just as in
...
You may call additional files with a rudimentary I<#include file>
...
The VendRoot directory, specified in the main program C<minivend>, is
the default location of all of the MiniVend program, configuration,
special, and library files. Unless changed in C<minivend>, the main
MiniVend server configuration file will be minivend.cfg in the VendRoot
directory.

If no I<Catalog> directives are present, only the special MiniVend
admin catalog will be started.
